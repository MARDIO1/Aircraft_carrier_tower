...
当前自动化打表功能需要优化，当前实现的功能中没有已有的servo参数存进表格，限幅的数据也没有传入表格，我已经在表格列好了标头，还需要配合你什么把相应功能完善；2是我希望格数和风扇转速两项可以在ui中保存并且上传到表格中，具体是ui中有一个新的位置是格数，这个位置不需要和地面站本身有任何关系他只是一个物理的真实参数，我希望在ui中可以输入这个格数并且保存，上传到表格中；风扇转速也是一样的，风扇转速则不一样，它虽然和地面站有关，但是当前可以保存一个自定义转速的快捷按钮，如果在此时点击上传到表格的按钮则可以直接把这个快捷的转速上传到表格，而不是固定的1500
